...
const ItemPage = ({
  ...
  const [getRooms, { loading: roomsLoading, data: roomsData }] = useLazyQuery(
    GET_ROOMS,
    {
      onCompleted: (data) => {
  ...
  const [updateWishList, { error: updateError }] = useMutation(
    UPDATE_WISH_LIST,
    {
  ...
  if (isWished) {
    actionButton = (
      <button
        className="item-page-top__content-jumbo-button item-page-top__content-jumbo-button__filled button"
        onClick={() => {
    ...
    actionButton = (
      <button
        className="item-page-top__content-jumbo-button button"
        onClick={() => {
          if (!isLoggedin) {
            dispatch({ type: 'TOGGLE_LOGIN_MODAL' });
            return;
  ...
  if (rooms && user) {
    ...
    for (const room of rooms) {
      if (
        roomMembers.includes(room.from.uid) &&
        roomMembers.includes(room.to.uid)
      ) {
        chatStarted = true;
        break;
      }
    ...
    <Container className="item-page">
      ...
      <Row className="item-page-top">
        ...
        <Col lg={5} className="pl-4">
          <div className="item-page-top__content">
            ...
            <div className="item-page-top__content-price">
              ${item.price}{' '}
              {item.shipping && (
            ...
            <div className="item-page-top__content-description">
              <p className="item-page-top__content-description__title">
                Description
              </p>
              <p className="item-page-top__content-description__textarea">
                {' '}
                {item.description}
              </p>
            </div>
      ...
      <Row className="item-page-bottom">
          ...
          <div className="item-page-bottom__seller">
            ...
            <div className="d-flex align-items-center mb-3 mt-3">
              ...
              <div className="item-page-bottom__seller-detail">
                <p className="item-page-bottom__seller-detail__name">
                  {item.createdby.username}
                </p>
                <p className="item-page-bottom__seller-detail__lists">
                  <b>63</b> items listed
                </p>
              </div>
            </div>
        ...
        <Col lg={9}>
          <div className="item-page-bottom__message">
            <p className="item-page-bottom__message-title">
              Message the seller
            </p>
            <ul className="item-page-bottom__message-options">
              <li onClick={() => setChatMessage('Is it still available?')}>
                Is it still available?
              </li>
              <li onClick={() => setChatMessage('Is the price negotiable?')}>
                Is the price negotiable?
              </li>
              <li onClick={() => setChatMessage('What condition is it in?')}>
      ...
      <div className="item-page-recommendations">
        ...
        <Row className="item-page-recommendations__row">
          {recommendations.map(
            (item) => item && <ItemCard post={item} key={item.pid} md={3} />
          )}
        </Row>
      </div>
    </Container>
