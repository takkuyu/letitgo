...
const Profile = () => {
  ...
  const { loading: postsLoading, data: postsData } = useQuery(
    GET_POSTS_BY_USER,
    {
      variables: { uid: user.uid },
      fetchPolicy: 'no-cache',
  ...
  if (postsLoading || wishListLoading) return <Spinner />;
      ...
      <div className="rounded overflow-hidden">
        <div className="pt-0 pb-4 cover">
          <div className="media align-items-end profile-head">
            <div className="profile mr-3">
              ...
              />
              <button className="btn-under-construction btn btn-outline-dark btn-sm btn-block">
                Edit profile
              </button>
            </div>
          ...
          <ul className="list-inline mb-0">
            <li className="list-inline-item">
              <h5 className="font-weight-bold mb-0 d-block">
                {postsData && postsData.getPostsByUser.length}
              </h5>
              <small className="text-muted">items listed</small>
            </li>
            <li className="list-inline-item">
              <h5 className="font-weight-bold mb-0 d-block">
                {wishListData && wishListData.getWishList.length}
              </h5>
              <small className="text-muted">in wish list</small>
            </li>
          </ul>
        </div>
        ...
        <Nav className="nav-tabs">
          <NavItem className="mr-3">
            <p
              className={`nav-link ${
                activeNav === NAV_OPTIONS.WISH ? 'active' : ''
              }`}
              onClick={() => setActiveNav(NAV_OPTIONS.WISH)}
            >
              Wish List
            </p>
          </NavItem>
          <NavItem className="ml-3">
            <p
              className={`nav-link ${
                activeNav === NAV_OPTIONS.RECENT ? 'active' : ''
              }`}
              onClick={() => setActiveNav(NAV_OPTIONS.RECENT)}
            >
              Your Items
            </p>
          </NavItem>
