...
class Favorite extends Component {
  ...
  async getLikesById(favorites) {
    const res = await Promise.all(
      favorites.map((favorite) =>
        axios
          .get('http://localhost:3000/postings/' + favorite, {
            headers: {
              Authorization: `Bearer ${sessionStorage.getItem('token')}`,
            },
          })
          .then((resp) => resp.data.posting)
      )
    );
    // this.setState({
    //     likes: res
  ...
  deleteLike(id) {
    axios
      .post(
        'http://localhost:3000/users/unliked',
        { deleteId: id },
        {
          headers: {
            Authorization: `Bearer ${sessionStorage.getItem('token')}`,
