...
export const selectPosts = createSelector([selectPostings], (postings) => {
...
export const selectPostsByCategory = (category, filters = {}) =>
  createSelector([selectPosts], (posts) => {
    ...
    filteredPosts = posts.filter(
      (post) => post.category === formatted_category
    );
...
function filterItems(items, filters) {
  for (let k of Object.keys(filters)) {
    const filterType = filterLists.filter(
      (filter) => filter.name.toLowerCase() === k
    )[0];
    const filterValue = filterType.lists.filter(
